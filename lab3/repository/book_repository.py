...
def unpack_cursor(cursor: str) -> dict:
...
async def page_after(
    ...
    cursor_data = unpack_cursor(cursor) if cursor else {}
    ...
    if has_next and books:
        last = books[-1]
        if sort_by == "title":
            next_cursor = pack_cursor({"title": last.title, "id": last.id})
        elif sort_by == "year":
            next_cursor = pack_cursor({"year": last.year, "id": last.id})
        else:
            next_cursor = pack_cursor({"id": last.id})

    return books, next_cursor


async def find(db: AsyncSession, book_id: str) -> Optional[Book]:
    result = await db.execute(select(Book).where(Book.id == book_id))
    return result.scalar_one_or_none()


async def insert(db: AsyncSession, book_data: dict) -> Book:
    book = Book(**book_data)
    db.add(book)
    await db.commit()
...
async def remove(db: AsyncSession, book_id: str) -> bool:
